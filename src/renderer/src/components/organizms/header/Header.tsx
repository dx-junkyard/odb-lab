...
import { GlobalUserMenu } from '@renderer/components/atoms/global-menu/user/GlobalUserMenu'
import { LoginModal } from '@renderer/components/molecules/user/login-modal/LoginModal'
import LogoutMenu from '@renderer/components/molecules/user/logout-menu/LogoutMenu'
import { GlobalVanillaMenu } from '@renderer/components/atoms/global-menu/vanilla/GlobalVanillaMenu'
  ...
  return (
    <header className="flex items-center header bg-white text-black w-full fixed z-10">
      <a href={'/'} className="flex items-center">
        <img src="/logo.svg" alt={'logo'} width={54} height={54} />
      ...
      <button className="space-y-2 md:hidden" onClick={handleMenuOpen}>
        ...
        />
        <span
          className={'block duration-300' + (isOpen ? ' opacity-0' : ' w-8 h-0.5 bg-gray-600')}
        />
        <span
          className={
            'block w-8 h-0.5 bg-gray-600 duration-300' +
