...
export const postPrompt = async (
  ...
  const response = await fetch('http://0.0.0.0:58912/api/interpreter/chat', {
    method: 'POST',
    body
  })

  if (response.status !== 200) {
    arg.updateLastMessage('AIとの接続に失敗しました。時間をおいて試してください。')
  }
  ...
  while (true) {
    ...
    const messageList = value.split('\n').map((line) => {
      ...
      try {
        ...
        if (json.content) {
          return json.content
        } else if (json['end'] && json['type'] === 'code') {
          return '\n```\n'
        } else if (json['start'] && json['type'] === 'code') {
          return '\n```python\n'
...
export const useInputPrompt = (
  ...
  const actionUsePrompt = async () => {
    ...
    setMessages((prev) => [
      ...
      {
        role: 'user',
        content: prompt + (fileList.length > 0 ? `\n\n添付ファイル名 : ${fileList[0].name}` : ''),
        datetime: new Date().toLocaleString()
      }
    ])

    const extensionPrompt =
      fileList.length > 0 ? `ファイルの拡張子:${fileList[0].name.split('.').pop()}\n\n` : ''

    try {
      await trigger({
        file: fileList[0],
        prompt:
          (instruction.length > 0 ? instruction + '\n---\n' + prompt : prompt) + extensionPrompt,
        setMessages,
        isFirst,
        updateLastMessage,
